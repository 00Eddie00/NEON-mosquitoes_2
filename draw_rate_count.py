...
def draw_rate(point, csv_file_path):
    ...
    # 绘制右侧纵坐标轴（温度）
    ax2.plot(time, wind_speed.fillna(0.0), 'r-', label='wind_speed')
    ax2.set_ylabel('wind_speed (m/s)', color='r')
    ax2.tick_params('y', colors='r', labelsize=7)
...
def main():
    ...
    draw_count(point, "count_rate.csv")
    draw_rate(point, "count_rate.csv")
